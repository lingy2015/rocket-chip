...
class SBToTL(implicit p: Parameters) extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val rdLegal_addr = edge.manager.supportsGetSafe(io.addrIn, io.sizeIn)
    val wrLegal_addr = edge.manager.supportsPutFullSafe(io.addrIn, io.sizeIn)
    val (rdLegal,  gbits) = edge.Get(0.U, io.addrIn, io.sizeIn)
    val (wrLegal, pfbits) = edge.Put(0.U, io.addrIn, io.sizeIn, muxedData)

    io.rdLegal := rdLegal && rdLegal_addr
    io.wrLegal := wrLegal && wrLegal_addr
